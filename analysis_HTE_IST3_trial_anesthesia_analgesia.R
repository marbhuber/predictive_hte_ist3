...
mycol = RColorBrewer::brewer.pal(n = 8, name = 'Dark2')
...
figA <- df %>% 
  ...
  geom_density_ridges(
    ...
    alpha = .6, color = "white", height=0.3
  )+
...
figB <- ggplot()+
  ...
  geom_errorbar(data = df.benefits,aes(x=risk,y=mymean,ymin=mylower,ymax=myupper,color=Subgroup),width=0.012,alpha=0.7)+
...
ggsave("figure1_13june2025.jpeg",dpi=600,width=8,height=8)
