...
),file = "table1_raw_10june2025.docx"
)
...
export2word(
  ...
  ),file = "table1_raw_10june2025_ci.docx"
)
...
# it includes treatment variable, but baseline risk will be calculated by setting treatment to zero (Placebo)

mymod <- glm(outcome ~ treatment.factor+
               age+
               sex+
               livealone_rand+
               infarct+
...
save_as_docx(gtsummary::tbl_regression(mymod,exponentiate=T) %>% 
  ...
  path = "regression_model_raw_10june25.docx")

####################################################################
# From now - following Steyerberg's analysis - use this fitted model
risk.mod <- lrm(outcome ~ treatment.numeric+
                 age+
                 sex+
                 livealone_rand+
                 infarct+
...
# calculate the linear predictor of baseline risk and attach to data frame
linear.predictor.baseline <- 
  risk.mod$coefficients[1]+ # Intercept
  0*risk.mod$coefficients[2]+ # For baseline risk
  risk.mod$x[,-1] %*% risk.mod$coefficients[-(1:2)] # all other terms
...
df$predictions                <- plogis(as.vector(linear.predictor.baseline))#predict(f,type="fitted.ind")

########################################################
########################################################
# Evalute interaction of treatment with linear predictor
...
ggsave("figure1_10june2025.jpeg",dpi=600,width=8,height=8)
